...
from assemblyai_agents.byo import Memo, Responder, Turn, digits_said
...
@responder.stage("identify", until=verified)
def identify(turn: Turn):
    ...
    if pending and pending.name == "find_patient":
        if not pending.get("found"):
            return turn.say(f"I could not find a record for that reference. {REFERENCE_ASK}")
        return turn.say(NAME_ASK)
    ...
    if pending and pending.name == "verify_caller":
        ...
        if problem == "no_name_heard":
            ...
            return turn.say(f"Sorry, I need the name on the record first. {NAME_ASK}")
        if problem == "no_record_in_context":
            return turn.say(REFERENCE_ASK)
    ...
    if reference(turn) or turn.preconnect:
        return turn.call("verify_caller", caller_said=said, reference=reference(turn))

    # Callers read a reference out in words as often as in figures.
    if len(digits_said(said)) >= 4:
        return turn.call("find_patient", reference_said=said)
    return turn.say(REFERENCE_ASK)
...
@responder.stage("notice", until=lambda turn: memo.has(call_key(turn), "notice"))
def notice(turn: Turn):
    ...
    return turn.say(f"Thank you, {first_name}. {NOTICE}{owing} {WHEN_ASK}")
...
@responder.stage("book", until=told_them)
def book(turn: Turn):
    ...
    if pending and pending.name == "find_appointments":
        ...
        if not slots:
            return turn.say("I have nothing free in the next three weeks. Shall I have someone call you?")
        offered = "; or ".join(slot["spoken"] for slot in slots[:2])
        return turn.say(f"I can offer {offered}. Would either of those work?")
    ...
    if pending and pending.name == "book_appointment":
        ...
        if pending.get("problem") == "slot_taken":
            return turn.call("find_appointments")
        return turn.say("I could not book that. Shall I have someone from the team call you back?")

    if pending and pending.name == "request_callback":
        memo.note(call_key(turn), "closed")
        return turn.say(f"That is with the team and they will call you. {CLOSING}")



    if not said:
        return turn.say(WHEN_ASK)
    ...
    if _TIME_WORDS.search(said) or (turn.said_before(WHEN_ASK) and not offered):
        return turn.call("find_appointments", preference_said=said)
    ...
    if wanted == "clinical_question":
        return turn.call("request_callback", reason="clinical question", note=said, reference=reference(turn))
...
def _position(turn: Turn, key: str, said: str):
    ...
    return turn.say(model.deliver(position, said) or f"{position} {WHEN_ASK}")
...
def close(turn: Turn):
    ...
    return turn.say(CLOSING)


def decide(turn: Turn):
    return responder.decide(turn)
